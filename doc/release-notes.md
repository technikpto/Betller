...
Betller Core is extensively tested on multiple operating systems using the Linux kernel, macOS 10.10+, and Windows 7 and later.

Microsoft ended support for Windows XP on [April 8th, 2014](https://www.microsoft.com/en-us/WindowsForBusiness/end-of-xp-support), No attempt is made to prevent installing or running the software on Windows XP, you can still do so at your own risk but be aware that there are known instabilities and issues. Please do not report issues about Windows XP to the issue tracker.

Apple released it's last Mountain Lion update August 13, 2015, and officially ended support on [December 14, 2015](http://news.fnal.gov/2015/10/mac-os-x-mountain-lion-10-8-end-of-life-december-14/). Betller Core software starting with v3.2.0 will no longer run on MacOS versions prior to Yosemite (10.10). Please do not report issues about MacOS versions prior to Yosemite to the issue tracker.

Betller Core should also work on most other Unix-like systems but is not frequently tested on them.
...
Enabling major improvements over the keystore management, the Betller wallet doesn't require regular backups as before, keys are following a deterministic creation path that can be verified at any time (before HD Wallet, every keypair was randomly created and added to the keypool, forcing the user to backup the wallet every certain amount of time or could end up loosing coins forever if the latest `wallet.dat` was not being used).
...
An extended description of this large work can be found in the PR [here](https://github.com/Betller-Project/Betller/pull/1327).
...
Automatic zPIV backup has been disabled. Thus, the following configuration options have been removed  (either as entries in the betller.conf file or as startup flags):
- `autozpivbackup`
- `backupzpiv`
- `zpivbackuppath`
...
The minimum required version of QT has been increased from 5.0 to 5.5.1 (the [depends system](https://github.com/betller-project/betller/blob/master/depends/README.md) provides 5.9.7)
...
- The output of `getstakingstatus` was reworked. It now shows the following information:
  ```
  {
     ...
     "staking_enabled": true|false,      (boolean) whether staking is enabled/disabled in betller.conf
     "coldstaking_enabled": true|false,  (boolean) whether cold-staking is enabled/disabled in betller.conf
...
The p2p alert system has been removed in [PR #1372](https://github.com/Betller-Project/Betller/pull/1372) and the 'alert' message is no longer supported.

### GUI

**Keyboard navigation**: dialogs can now be accepted with the `ENTER` (`RETURN`) key, and dismissed with the `ESC` key ([#1392](https://github.com/Betller-Project/Betller/pull/1392)).
...
Addresses in the dropdown of the "Send Transaction" and "Send Delegation" widgets are now automatically sorted by label with ascending order ([#1393](https://github.com/Betller-Project/Betller/pull/1393)).


**Custom Fee**: The custom fee selected when sending a transaction is now saved in the wallet database and persisted across multiple sends and wallet's restarts ([#1406](https://github.com/Betller-Project/Betller/pull/1406)).


**Include delegations in send**: The send and cold-staking page present a checkbox ([#1391](https://github.com/Betller-Project/Betller/pull/1391)) to make the automatic input selection algorithm include delegated (P2CS) utxos if needed. The option is unchecked by default.


**Staking Charts**: can now be hidden at startup (with a flag `--hidecharts`) or at runtime with a checkbox in settings --> options --> display ([PR #1475](https://github.com/Betller-Project/Betller/pull/1475)).
...
__Context Lock/Unlock__ [[PR #1387](https://github.com/Betller-Project/Betller/pull/1387)]:<br>
...
- The `-reservebalance` configuration/startup option has been removed ([PR #1373](https://github.com/Betller-Project/Betller/pull/1373)).
