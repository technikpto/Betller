...
*Setup instructions for a gitian build of Betller Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Betller
...
to the Betller GitHub release page.
...
- [Building Betller Core](#building-betller-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/betller-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/betller-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/betller-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/betller-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/betller-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/betller-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/betller-project/gitian.sigs/) repository:
...
```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/betller-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/betller-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/betller-osx-*-build.assert
```
