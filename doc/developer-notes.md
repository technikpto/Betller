...
- [Development guidelines](#development-guidelines)
    - [General Betller Core](#general-betller-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Betller Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Betller Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/betller-project/betller/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_betller
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_betller --log_level=test_suite
$ valgrind -v --leak-check=full src/betllerd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_betller.coverage/index.html`.
```
...
However, in open source software such as Betller Core, where everyone uses
...
pay attention to for reviewers of Betller Core code.

General Betller Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Betller Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Betller Core so that they can be integrated
...
file descriptors it will cause problems with Betller's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Betller-Project/Betller.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `betller-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `betller-cli` and the GUI debug console use this table to determine how to
