...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Betller";
#else
    fs::path pathRet;
...
{
    fs::path pathConfigFile(GetArg("-conf", "betller.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty betller.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override betller.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", "betllerd.pid"));
