    ...
    ui(new Ui::RequestDialog)
{
    ...
    ui->labelMessage->setText(tr("Instead of sharing only a BTLLR address, you can create a payment request, bundling up more information."));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 BTLLR");
...
void RequestDialog::onNextClicked(){
    if(walletModel) {
        ...
        if (this->isPaymentRequest) {
            ...
            title = "Request for " + BitcoinUnits::format(displayUnit, value, false, BitcoinUnits::separatorAlways) + " BTLLR";
        } else {
