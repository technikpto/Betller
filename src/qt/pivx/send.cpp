    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer BTLLR"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("BTLLR address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 BTLLR");
...
void SendWidget::refreshView()
{
    ...
    ui->pushButtonSave->setText(tr("Send BTLLR"));
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend BTLLR (not zRPD)
...
void SendWidget::onCoinControlClicked()
{
    if (walletModel->getBalance() > 0) {
        ...
        inform(tr("You don't have any BTLLR to select."));
    }
}
