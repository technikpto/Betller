    ...
    isLoading(false)
{
    ...
    ui->labelSubtitle2->setText(tr("Accept BTLLR delegation / Delegate BTLLR"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select BTLLR outputs to delegate.");
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 BTLLR" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
